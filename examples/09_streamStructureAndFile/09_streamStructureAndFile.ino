/*
 * EBYTE RF E70
 * Send a Stream (File) and a structure
 *
...
//
	  ...
	  if (SPIFFS.begin(true)) {
	      ...
	      ftpSrv.begin("user","passord");    //username, password for ftp.   (default 21, 50009 for PASV)
	  }
...
void loop() {
  ...
  if (Serial.available()) {
		...
		if (input.length()>1){
			...
			if (!SPIFFS.exists	(input)) {
			    Serial.println("There was an error opening the file for reading");
			} else  {
				File inputFile = SPIFFS.open(input);

				char fic[10];
				uint16_t s = inputFile.size();

				struct FileInfoMsg fi = { {0}, 0 };
				strcpy( fi.filename, input.c_str());
				fi.size = s;

				Serial.println(input);
				Serial.println(inputFile.size());


				Serial.println(fi.filename);
				Serial.println(fi.size);

				ResponseStatus rs = e70ttl.streamStructMessage(&fi, sizeof(FileInfoMsg), &inputFile);
